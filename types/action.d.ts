...
declare global {
  const BarItems: { [id: string]: BarItem };
  ...
  enum KeybindCondition {
    Always = 'always',
    UnlessCtrl = 'unless_ctrl',
    UnlessShift = 'unless_shift',
    UnlessAlt = 'unless_alt',
  }

  enum KeybindModifierKey {
    Ctrl = 'ctrl',
    Shift = 'shift',
    Alt = 'alt',
    Meta = 'meta',
  ...
  type KeybindModifier = KeybindCondition | KeybindModifierKey;

  /** An object representing a key + modifier + variation that when pressed at the same time perform a set action.
   *
   * eg: ctrl+c (copy), <cond: unless-shift> [ctrl]+v (paste-with-format) */
  class Keybind implements KeybindKeys {
    /** Create a keybind */
    constructor(keys: KeybindKeys, variations?: Record<string, KeybindModifier>);

    /** See {@link KeybindKeys.key} */
    key: number | string;
    /** See {@link KeybindKeys.ctrl} */
    ctrl: boolean;
    /** See {@link KeybindKeys.shift} */
    shift: boolean;
    /** See {@link KeybindKeys.alt} */
    alt: boolean;
    /** See {@link KeybindKeys.meta} */
    meta: boolean;
    /**  */

    /**  */
    variations?: { [key: string]: { name: KeybindModifier; description?: string } };

    /** Set the assigned key ; this keybind is now set to describe the provided key combo. {@link hasKey} now returns true. */
    set(keys: KeybindKeys): this;

    /** Clear the assigned key ; this keybind is no longer set to a specific key combo. {@link hasKey} now returns false. */
    clear(): this;
    ...
    // TODO: What is a sub keybind?
    // TODO: returns `this | undefined` ; when and why for each?
  ...
  class Action extends BarItem {
    ...
    // TODO: I'm not sure what to do about this error...
    conditional?(): boolean;
    /**
  ...
  type NumSliderOptions = WidgetOptions & {
    private?: boolean;
    settings?: { default?: number; min?: number; max?: number; interval?: number; step?: number };
  ...
  class BarText extends Widget {
    constructor(id: string, options: WidgetOptions & { text: string });
    set(text: any): this;
    update(): this;
    trigger(event: Event): boolean;
  }
